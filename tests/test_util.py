...
def test_rcont_bad_input(method):
    ...
    # wrong dimension
    with pytest.raises(ValueError):
        rcont(5, np.arange(4).reshape((2, 2)), c, method, rng)

    # wrong dimension
    with pytest.raises(ValueError):
        rcont(5, r, np.arange(4).reshape((2, 2)), method, rng)

    # degenerate table
    with pytest.raises(ValueError):
        rcont(5, np.arange(1), np.arange(2), method, rng)

    # degenerate table
    with pytest.raises(ValueError):
        rcont(5, np.arange(2), np.arange(1), method, rng)

    # negative entries
    r2 = r.copy()
    r2[0] = -1
    with pytest.raises(ValueError):
        rcont(5, r2, c, method, rng)

    # negative entries
    c2 = c.copy()
    c2[-1] = -1
    with pytest.raises(ValueError):
        rcont(5, r, c2, method, rng)

    # sum(r) != sum(c)
    with pytest.raises(ValueError):
        rcont(5, r + 1, c, method, rng)

    # total is zero
    with pytest.raises(ValueError):
        rcont(5, np.zeros(2), np.zeros(3), method, rng)
